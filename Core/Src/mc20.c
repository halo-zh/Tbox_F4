...
//#define EUROPE
#ifdef EUROPE
#define ProductKey     "a1QT12UnGbp"             //2ú?·KEY
#define DeviceName     "86069705912568"      //
#define DeviceSecret    "3b1ef7867b2a782d5f9ab7a4a8f75a0a"  //
#define PeriodicTopic         "/sys/a1QT12UnGbp/86069705912568/thing/event/property/post"
#define BatEventTopic        "/sys/a1QT12UnGbp/86069705912568/thing/event/BatErr/post"
#define InverterEventTopic        "/sys/a1QT12UnGbp/86069705912568/thing/event/InverterErr/post"
#else
...
START:  
   ...
   ret = mc20_normal_check("ATE0\r",50);
   
   ret = mc20_normal_check("AT+QISCON?\r",300);
   
   ret = mc20_normal_check("AT+QIDNSIP=1\r",300);
    
   
   ret = mc20_normal_check("AT+QISCON?\r",300);
   
   // ret = mc20_normal_check("AT+COPS=0,1,\"\"\r",10000);

    osDelay(10);
    
    
   //  ret = mc20_normal_check("AT+COPS=?\r",20000);
     
   //   ret = mc20_normal_check("AT+QICLOSE=0\r",10000);

    

     // ret = mc20_normal_check("AT+QICSGP=1,\"TM\",\"\",\"\",1\r",300);

      osDelay(10);
      
    //  ret = mc20_normal_check("AT+QICSGP=1\r",300);
      
    iter = 0;
    while(1) {
        mc20_normal_check("AT+CSQ\r",300);
...
void GPSInit()
{
    ...
    ret = mc20_normal_check("AT+QIFGCNT=2\r");
    if(ret != AT_SUCCESS) {
    ...
    ret = mc20_normal_check("AT+QGNSSEPO=1\r");
    if(ret != AT_SUCCESS) {
        return;
    }
    osDelay(10);
 ...
 */   
    ...
    ret = mc20_normal_check("AT+QGNSSC=1\r");
    if(ret != AT_SUCCESS) {
...
uint32_t batSOC,batSOH,throttlePercent,motorRPM,batVolt,batCurrent;
...
uint16_t packInverterErrData(char *t_payload)
{
    ...
    memset(inverterErrorInfo,0,1000);
    //uint16_t pkt_id = 1;
...
int MQTT_CFG()
{
  ...
  int ret = user_get_data_with_delay(buffer, BUF_LEN,300);
...
{
#ifdef EUROPE
  // char txbuffer[400]="AT+QMTOPEN=0,\"iot-as-mqtt.eu-central-1.aliyuncs.com\",1883\r\n";
   sprintf(txbuffer,"AT+QMTOPEN=0,\"a1QT12UnGbp.iot-as-mqtt.eu-central-1.aliyuncs.com\",1883\r\n");
#else
   sprintf(txbuffer,"AT+QMTOPEN=0,\"iot-as-mqtt.cn-shanghai.aliyuncs.com\",1883\r\n");
   //char txbuffer[400]="AT+QMTOPEN=0,\"iot-as-mqtt.cn-shanghai.aliyuncs.com\",1883\r\n"; 
#endif
   printf("%s",txbuffer);
   user_send_data_with_delay(txbuffer);  
...
int MQTT_Connect()
{
  ...
  char txbuffer[300] = "AT+QMTCONN=0,\"scootertest1\"\r\n";
  printf("%s",txbuffer);
  user_send_data_with_delay(txbuffer);  
...
{
    char txbuffer[300];
    sprintf(txbuffer,"AT+QMTPUB=0,0,0,0,\"%s\",%d\r\n",topic,len);
    printf("%s",txbuffer);
...
uint8_t waitResetCnt=0;
int MQTT_Disconnect()
{
  
  char txbuffer[100]="AT+QMTDISC=0\r\n";
  user_send_data_with_delay(txbuffer);  
...
{
     int ret;
     memset(mqtt_pub_data,0,sizeof(mqtt_pub_data));
